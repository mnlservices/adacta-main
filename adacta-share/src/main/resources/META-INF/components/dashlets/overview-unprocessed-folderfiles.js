if (typeof ADACTA === undefined || !ADACTA) {
     ...
     * @return {ADACTA.OverviewUnprocessedFolderFiles} The new ManagePersonnelFiles instance
     * @constructor
     */
    ADACTA.OverviewUnprocessedFolderFiles = function(htmlId) {
        ADACTA.OverviewUnprocessedFolderFiles.superclass.constructor.call(this, "ADACTA.OverviewUnprocessedFolderFiles", htmlId, ["button", "container", "datasource", "datatable", "paginator"]);
        this.searchText = "";
        return this;
    }

    YAHOO.extend(ADACTA.OverviewUnprocessedFolderFiles, Alfresco.component.Base, {
        ...
        renderCellIcon: function MPF_renderCellIcon(elCell, oRecord, oColumn, oData) {
            ...
            if (oRecord.getData("isContentType") == false) {
                var parts = uri.split("folder-details");
                var params = "panel=viewFolder&nodeRef=" + oRecord.getData("nodeRef");
                var part = "adacta-index#state=" + encodeURIComponent(params);
                uri = parts[0] + part;
            }

            elCell.innerHTML = '<span class="icon32"><a href="' + uri + '"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/images/filetypes/' + Alfresco.util.getFileIcon(oRecord.getData("name"), oRecord.getData("nodeType"), 16) + '" alt="' + $html(name) + '" /></a></span>';
        },
        ...
        renderCellCreatorFullName: function renderCellModified(elCell, oRecord, oColumn, oData) {
        	// If there is a non-empty scan employee value, then this is the relevant information for this cell
        	if (oRecord.getData("scanEmployeeFullName") != null && oRecord.getData("scanEmployeeFullName") != "") {
        		elCell.innerHTML = $html(oRecord.getData("scanEmployeeFullName"));
        	} else {
        ...
        renderCellActions: function MPF_renderCellActions(elCell, oRecord, oColumn, oData) {
            ...
            var desc = "";
            desc += '<a class="onEditNode" title="' + this.msg("actions.document.edit-metadata") + '"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/actions/document-edit-metadata-16.png" title="' + this.msg("actions.document.edit-metadata") + '"/></a>&nbsp;';

            if (oRecord.getData("isContentType")) {
                desc += '<a class="onDeleteNode" title="' + this.msg("actions.document.delete") + '"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/actions/document-delete-16.png" title="' + this.msg("actions.document.delete") + '"/></a>&nbsp;';
            } else {
                desc += '<a class="onDeleteNode" title="' + this.msg("actions.folder.delete") + '"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/documentlibrary/actions/folder-delete-16.png" title="' + this.msg("actions.folder.delete") + '"/></a>&nbsp;';
            }
