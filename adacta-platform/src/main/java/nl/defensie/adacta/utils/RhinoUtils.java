...
/**
 * Utility methods for converting data between Mozilla Rhino representation and standard Java collections.
 *
	...
	public static int getInteger(Map<String, Object> map, String key, int defaultValue) {
		...
		if (map.get(key) != null) {
			if (map.get(key) instanceof Number) {
				...
				if (result < 0) {
					throw new IllegalArgumentException(key + " must be a positive number, but is instead: " + map.get(key));
				}
			} else {
	...
	public static List<Object> getArray(Map<String, Object> map, String key) {
		...
		if (value != null) {
			if (value instanceof NativeArray) {
				...
				throw new IllegalArgumentException(key + " must be a NativeArray or Object[], but is instead: " + value);
			}
		}
		return result;
	}
		...
		if (map.get(key) != null) {
			if (map.get(key) instanceof NativeJavaObject) {
				...
				if (o instanceof ScriptNode) {
					...
					throw new IllegalArgumentException(key + " must have a ScriptNode as Java object, but has instead: " + o);
				}
			} else {
